...
function plotScenario(p, s)
    % Presentation-quality plot for one selected site/year scenario.

    if ~exist("simulink_figures", "dir")
        mkdir("simulink_figures");
    end
    ...
    % Presentation colors
    cBase = [0.10 0.35 0.75];       % blue
    cUnmanaged = [0.85 0.20 0.15];  % red
    cManaged = [0.95 0.65 0.10];    % gold
    cCapacity = [0.35 0.20 0.55];   % purple
    cEV = [0.10 0.60 0.45];         % green

    fig = figure("Name", "Presentation Site-Level Load Behavior", ...
                 "Color", "w", ...
                 "Units", "pixels", ...
    ...
    %% TOP GRAPH: Total site load
    nexttile;
    hold on; grid on; box on;

    % Shade overload area if unmanaged charging exceeds capacity.
    ...
    if any(overloadIdx)
        ...
        fill(xFill, yFill, cUnmanaged, ...
            "FaceAlpha", 0.16, ...
            "EdgeColor", "none", ...
            "DisplayName", "Unmanaged overload area");
    end

    plot(p.t_hr, p.baseLoad_kW, ...
        "Color", cBase, ...
        "LineWidth", 2.4, ...
        "DisplayName", "Base building load");

    plot(p.t_hr, p.totalUnmanaged_kW, ...
        "Color", cUnmanaged, ...
        "LineWidth", 2.8, ...
        "DisplayName", "Total load without managed charging");

    plot(p.t_hr, p.totalManaged_kW, ...
        "Color", cManaged, ...
        "LineWidth", 3.2, ...
        "DisplayName", "Total load with managed charging");

    plot(p.t_hr, p.capacity_kW, ...
        "--", ...
        "Color", cCapacity, ...
        "LineWidth", 2.8, ...
        "DisplayName", "Site capacity limit");

    xlabel("Hour of day", "FontWeight","bold");
    ylabel("Power demand (kW)", "FontWeight","bold");

    title("Site-Level EV Charging Load Behavior", ...
        "FontWeight","bold", ...
        "FontSize", 16);

    subtitle(siteLabel + " | Scenario: " + scenarioLabel + ...
    ...
    yMax = max([p.totalUnmanaged_kW; p.totalManaged_kW; p.capacity_kW]) * 1.12;
    ylim([0 yMax]);
    ...
    % Add result summary note.
    maxUnmanagedOverload = max(p.overloadUnmanaged_kW);

    if maxUnmanagedOverload > 0
        resultText = "Result: unmanaged charging exceeds capacity by " + ...
            string(round(maxUnmanagedOverload,1)) + ...
            " kW; managed charging controls the peak.";
        badgeColor = [1.00 0.92 0.88];
    else
        resultText = "Result: no overload in this scenario; managed and unmanaged loads stay below capacity.";
        badgeColor = [0.90 0.97 0.91];
    end

    annotation(fig, "textbox", [0.12 0.915 0.78 0.055], ...
        "String", resultText, ...
        "FitBoxToText","off", ...
    ...
    %% BOTTOM GRAPH: EV request vs managed EV output
    nexttile;
    hold on; grid on; box on;

    plot(p.t_hr, p.evRequest_kW, ...
        "Color", cUnmanaged, ...
        "LineWidth", 2.8, ...
        "DisplayName", "EV charging request");

    plot(p.t_hr, p.evManaged_kW, ...
        "Color", cEV, ...
        "LineWidth", 3.0, ...
        "DisplayName", "EV power allowed by managed charging");

    % Shade curtailed/delayed charging.
    ...
    if any(curtailIdx)
        ...
        fill(xFill, yFill, cUnmanaged, ...
            "FaceAlpha", 0.14, ...
            "EdgeColor", "none", ...
            "DisplayName", "Curtailed or delayed EV power");
    end

    xlabel("Hour of day", "FontWeight","bold");
    ylabel("EV charging power (kW)", "FontWeight","bold");

    title("EV Charging Request vs Managed Charging Output", ...
        "FontWeight","bold", ...
        "FontSize", 14);
    ...
    yMax2 = max([p.evRequest_kW; p.evManaged_kW]) * 1.22;
    ...
    safeSite = regexprep(char(s.site_name), '[^a-zA-Z0-9]+', '_');
    safeYear = regexprep(char(s.year), '[^a-zA-Z0-9]+', '_');

    outFile = fullfile("simulink_figures", ...
        "site_load_behavior_" + string(safeSite) + "_" + string(safeYear) + ".png");

    exportgraphics(fig, outFile, "Resolution", 300);

    fprintf("Saved presentation figure: %s\n", outFile);
end
...
function plotYearComparison(allProfiles, allScenarios)
    % Presentation-quality comparison across Now, 2035, and 2050.
    ...
    cNow = [0.10 0.35 0.75];
    c2035 = [0.85 0.35 0.10];
    c2050 = [0.95 0.65 0.10];
    cCapacity = [0.35 0.20 0.55];

    fig = figure("Name", "Presentation Year Comparison", ...
                 "Color", "w", ...
                 "Units", "pixels", ...
                 "Position", [120 120 1200 720]);

    tiledlayout(2,1, "TileSpacing","compact", "Padding","compact");

    %% TOP GRAPH: Managed total load by year
    nexttile;
    hold on; grid on; box on;
    ...
    for i = 1:numel(allProfiles)
        ...
        plot(p.t_hr, p.totalManaged_kW, ...
            "LineWidth", 3.0, ...
            "Color", colors{i}, ...
            "DisplayName", string(s.year) + ...
            " managed load (" + string(s.ports) + " ports)");
    end

    plot(allProfiles{1}.t_hr, allProfiles{1}.capacity_kW, ...
        "--", ...
        "LineWidth", 2.8, ...
        "Color", cCapacity, ...
        "DisplayName", "Site capacity limit");
    ...
    xlabel("Hour of day", "FontWeight","bold");
    ylabel("Total site load (kW)", "FontWeight","bold");

    title("Managed Charging Load Comparison Across Planning Years", ...
        "FontWeight","bold", ...
        "FontSize", 16);

    subtitle(siteLabel + ...
        " | Uses recommended port counts from the V2 right-sizing model");
    ...
    %% BOTTOM GRAPH: Peak load and overload summary
    nexttile;
    hold on; grid on; box on;
    ...
    b = bar(x, [peakUnmanaged peakManaged], "grouped");
    b(1).FaceColor = [0.85 0.20 0.15];
    b(2).FaceColor = [0.10 0.60 0.45];

    ylabel("Peak total load (kW)", "FontWeight","bold");

    yyaxis right;

    h = plot(x, maxOverload, "-o", ...
        "LineWidth", 2.8, ...
        "MarkerSize", 7, ...
        "Color", [0.35 0.20 0.55], ...
        "DisplayName", "Max unmanaged overload");

    ylabel("Max unmanaged overload (kW)", "FontWeight","bold");

    title("Peak Load and Overload Summary", ...
        "FontWeight","bold", ...
        "FontSize", 14);
    ...
    % Save high-resolution image automatically.
    safeSite = regexprep(char(allScenarios{1}.site_name), '[^a-zA-Z0-9]+', '_');

    outFile = fullfile("simulink_figures", ...
        "year_comparison_" + string(safeSite) + ".png");
